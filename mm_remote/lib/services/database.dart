...
class SqLite {
  ...
  void deleteAllDefaultCommands(String deviceName) async {
    var dbClient = await db;
    dbClient.delete('DefaultCommands',
        where: "deviceName = ?", whereArgs: [deviceName]);
  ...
  Future<List<String>> getDefaultCommands(String deviceName) async {
    ...
    var dbClient = await db;
    List<Map> list = await dbClient.query('DefaultCommands',
        where: "deviceName = ?", whereArgs: [deviceName]);
  ...
  Future updateApiKey(String deviceName, String apiKey) async {
    ...
    await dbClient.transaction((txn) async {
      return await txn
          .rawUpdate('UPDATE Devices SET apiKey = ? WHERE deviceName = ?', [
        apiKey,
        deviceName,
      ]);
    });
  ...
  Future updateMonitorStatusSetting(
      String deviceName, String monitorStatus) async {
    var dbClient = await db;
    await dbClient.transaction((txn) async {
  ...
  Future<MirrorStateArguments> getSettings(String deviceName) async {
    ...
    if (list.length >= 1) {
      setting = new MirrorStateArguments(
          list[0]["deviceName"],
          list[0]["brightness"],
          list[0]["alertDuration"],
          list[0]["monitorStatus"]);
    }
    return setting;
  }
}
